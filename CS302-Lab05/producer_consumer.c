...
#define SLEEP_TIME 1
#define SLEEP_CHANCE \
  4  // sleep for "SLEEP_TIME" seconds at a chance of "1 / SLEEP_CHANCE"
...
pthread_mutex_t mutex =
    PTHREAD_MUTEX_INITIALIZER;  // protect head, tail, and buffer
...
void drawing(int turn, int item) {
  ...
  for (i = count; i < N - 1; i++) sprintf(print_buf, "%s|   ", print_buf);
  sprintf(print_buf, "%s|", print_buf);
...
void *producer(void *nothing) {
  ...
  while (1) {
    ...
    if (rand_r(&s) % SLEEP_CHANCE == 0) sleep(1);
  }
}
